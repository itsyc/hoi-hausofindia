...
export default function AddProductPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Transform arrays back to expected formats
    const payload = {
      ...formData,
      ...
      specs: formData.specs.filter(curr => curr.key.trim() !== ''),
      variants: formData.variants.map(v => ({
        ...
        specs: v.specs.filter(curr => curr.key.trim() !== '')
      }))
    }
  ...
  return (
    <div className="container" style={{ padding: '40px 0', maxWidth: '1000px' }}>
      <h1 className="text-gold mb-4" style={{ fontSize: '2rem' }}>Add New Product</h1>
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
                ...
                <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '15px' }}>
                    <div style={{ flex: 1 }}>
                      ...
                      <input type="text" className="form-input" placeholder="e.g. HOI-C-43-1GB"
                        value={(v as any).sku || ''} onChange={e => {
                          const newV = [...formData.variants]; (newV[i] as any).sku = e.target.value; setFormData({...formData, variants: newV})
                        }} />
                    </div>
                    <div style={{ flex: 1 }}>
                      <label className="form-label text-gold">Barcode / EAN (for scanner software)</label>
                      <input type="text" className="form-input" placeholder="Scan or type barcode e.g. 8901234567890"
                        value={(v as any).barcode || ''} onChange={e => {
                          const newV = [...formData.variants]; (newV[i] as any).barcode = e.target.value; setFormData({...formData, variants: newV})
                        }} />
                    </div>
                    <div style={{ width: '130px' }}>
                      <label className="form-label">Stock Units</label>
                      <input type="number" className="form-input" placeholder="10"
                        value={(v as any).stockQuantity ?? 10} onChange={e => {
                          const newV = [...formData.variants]; (newV[i] as any).stockQuantity = Number(e.target.value); setFormData({...formData, variants: newV})
                        }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px' }}>
                    ...
                    <div>
                      ...
                      {/* Editable Base Specs */}
                      {formData.specs.filter(bs => bs.key.trim() !== '').length > 0 && (
                        <div style={{ marginBottom: '20px' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                            {formData.specs.filter(bs => bs.key.trim() !== '').map((bs, bsIdx) => {
                              const variantSpecIndex = v.specs.findIndex(vs => vs.key.trim().toLowerCase() === bs.key.trim().toLowerCase() && vs.key.trim() !== '')
                              const isOverridden = variantSpecIndex !== -1
                              const displayValue = isOverridden ? v.specs[variantSpecIndex].value : bs.value

                              return (
                                <div key={bsIdx} style={{ display: 'flex', gap: '10px' }}>
                                  <input type="text" className="form-input" disabled value={bs.key} 
                                    style={{ flex: 1, backgroundColor: 'rgba(255,255,255,0.02)', color: 'var(--text-secondary)' }} />
                                  <input type="text" className="form-input" 
                                    ...
                                    style={{ flex: 1, borderColor: isOverridden ? 'var(--accent-cyan)' : 'var(--border-color)' }} 
                                  />
                                  {isOverridden ? (
                                    <button type="button" onClick={() => {
                                      const newV = [...formData.variants]
                                      ...
                                      setFormData({...formData, variants: newV})
                                    }} style={{ background: 'none', border: 'none', color: '#ff4d4d', cursor: 'pointer', width: '30px', fontSize: '0.8rem' }} title="Revert to base">↺</button>
                                  ) : (
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                          {v.specs.filter(vs => !formData.specs.some(bs => bs.key.trim().toLowerCase() === vs.key.trim().toLowerCase() && bs.key.trim() !== '')).map((spec, sIndex) => {
                            // Find original index in the variant's specs array to update correctly
        ...
        <div className="premium-card" style={{ padding: '20px 30px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
            ...
            <label htmlFor="published" style={{ color: 'var(--text-primary)', fontSize: '1.1rem', cursor: 'pointer' }}>Publish to Storefront immediately?</label>
          </div>
          <button type="submit" className="btn-primary" disabled={loading} style={{ padding: '15px 40px', fontSize: '1.2rem' }}>
            {loading ? 'Creating...' : 'Create Complete Product'}
          </button>
        </div>
